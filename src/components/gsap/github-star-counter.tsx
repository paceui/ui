...
const fetchGitHubStars = async (repo: string): Promise<number> => {
...
export const GithubStarCounter = ({
    ...
    const [stars, setStars] = useState<number | string>(defaultStar);

    useEffect(() => {
        let isMounted = true;
        fetchGitHubStars(repo).then((stars) => {
            if (isMounted) setStars(stars);
        });
        return () => {
            isMounted = false;
        };
